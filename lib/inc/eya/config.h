...
#    define EYA_LIBRARY_OPTION_SHARED_BUILD EYA_CONFIG_OFF
#endif // EYA_LIBRARY_OPTION_SHARED_BUILD

/**
 * @def EYA_LIBRARY_OPTION_STATIC_BUILD
 * @brief Configuration macro for static library build mode
 *
 * @details Determines whether the library is being built as a static library.
 *          Defaults to `EYA_CONFIG_ON` if not explicitly defined (safe default).
 *
 * When enabled (`EYA_CONFIG_ON`):
 * - All symbols remain visible without special export/import attributes
 * - Follows traditional static linking behavior
 *
 * @note This is the recommended default for maximum compatibility.
 *       Disable only when explicitly building as shared library.
 *
 * @see EYA_ATTRIBUTE_SYMBOL
 */
#ifndef EYA_LIBRARY_OPTION_STATIC_BUILD
#    define EYA_LIBRARY_OPTION_STATIC_BUILD EYA_CONFIG_ON
...
#    define EYA_LIBRARY_OPTION_RUNTIME_TERMINATE_USE_STDLIB EYA_CONFIG_ON
