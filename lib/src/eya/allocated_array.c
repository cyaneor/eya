...
{
    return eya_memory_range_is_uninit(eya_ptr_rcast(const eya_memory_range_t, self))
               ? 0
               : eya_memory_typed_get_size(eya_ptr_rcast(const eya_memory_typed_t, self));
}
...
{
    eya_runtime_check(
        eya_memory_typed_is_equal_element_size(eya_ptr_rcast(eya_memory_typed_t, self),
                                               eya_ptr_rcast(eya_memory_typed_t, other)),
        EYA_RUNTIME_ERROR_DIFFERENT_ELEMENT_SIZE);

    eya_allocated_range_exchange(eya_ptr_rcast(eya_allocated_range_t, self),
                                 eya_ptr_rcast(eya_allocated_range_t, other));
}
...
eya_allocated_array_resize(eya_allocated_array_t *self, eya_usize_t size)
{
    ...
    const eya_usize_t element_size =
        eya_memory_typed_get_element_size(eya_ptr_rcast(eya_memory_typed_t, self));

    const eya_usize_t size_in_bytes = size * element_size;
    eya_allocated_range_resize(eya_ptr_rcast(eya_allocated_range_t, self), size_in_bytes);
}